...
import { 
  FiSearch, FiPlus, FiTrash2, FiUser, FiChevronRight, 
  FiChevronLeft, FiEdit, FiShoppingCart, FiTag, 
  FiClock, FiCheckCircle, FiAlertCircle, FiPrinter, 
  FiPhone, FiHome, FiX, FiDollarSign, FiCreditCard,
  FiPieChart, FiSettings, FiCalendar, FiClock as FiTime,
  FiUserPlus, FiGrid, FiMenu, FiPackage, FiCamera,FiMail
} from 'react-icons/fi';
...
const customers = [
    { name: "Molly Vaughan", phone: "(405) 555-0128", email: "molly@mail.com", vip: true, img: "https://i.pravatar.cc/150?img=1" },
    { name: "Kathrinee Moss", phone: "(209) 555-0104", email: "kath@mail.com", vip: false, img: "https://i.pravatar.cc/150?img=2" },
    { name: "Joshua Wilson", phone: "(270) 555-0117", email: "joshua@mail.com", vip: false, img: "https://i.pravatar.cc/150?img=3" },
    { name: "Erica Wyatt", phone: "(208) 555-0112", email: "erica@mail.com", vip: true, img: "https://i.pravatar.cc/150?img=4" },
    { name: "Zahir Mays", phone: "(307) 555-0133", email: "zahir@mail.com", vip: true, img: "https://i.pravatar.cc/150?img=5" },
  ];
  ...
  const items = [
    { id: 1, name: 'Fettuccini with Spinach', price: 15, status: 'Sold out', category: 'Dinner' },
    { id: 2, name: 'Black Bean Chili', price: 15, status: 'Trending', category: 'Lunch' },
    { id: 3, name: 'Spicy Healthy Sauté', price: 12.5, discount: 17.25, status: 'Running Low', category: 'Main Courses' },
    { id: 4, name: 'Spicy Black Bean Burger', price: 15, status: 'New', category: 'Lunch' },
    { id: 5, name: 'Avocado Toast', price: 9.5, status: 'Popular', category: 'Breakfast' },
    { id: 6, name: 'Chocolate Lava Cake', price: 8, status: '', category: 'Dessert' },
    { id: 7, name: 'Margherita Pizza', price: 14, status: '', category: 'Dinner' },
    { id: 8, name: 'Iced Caramel Latte', price: 5.5, status: '', category: 'Beverages' },
  ];
  ...
  const getStatusColor = (status) => {
    switch(status) {
      ...
      case 'Trending': return 'bg-gray-200 text-gray-800';
      case 'Running Low': return 'bg-gray-300 text-gray-800';
      case 'New': return 'bg-white text-gray-800 border border-gray-800';
  ...
  return (
    <div className="flex h-screen bg-white font-sans text-gray-900">
      {/* Sidebar */}
      <aside className={`${sidebarOpen ? 'w-64' : 'w-20'} bg-black text-white flex flex-col items-center py-6 transition-all duration-300 ease-in-out`}>
        ...
        <nav className="flex flex-col w-full px-4 gap-2">
          ...
          <button className="flex items-center gap-3 p-3 bg-white text-black rounded-lg font-medium">
            <FiShoppingCart size={20} />
            {sidebarOpen && <span>Orders</span>}
          </button>
          
          <button className="flex items-center gap-3 p-3 text-gray-300 hover:text-white hover:bg-gray-800 rounded-lg transition">
            <FiPieChart size={20} />
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        <header className="flex items-center justify-between px-8 py-4 bg-white border-b border-gray-200">
          <div className="flex gap-6 items-center">
            ...
            <button className="text-gray-600 hover:text-black transition font-medium">Reservations</button>
            <button className="text-gray-600 hover:text-black transition font-medium">Inventory</button>
            <button className="text-gray-600 hover:text-black transition font-medium">Staff</button>
          </div>
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          <section className="w-2/3 p-6 overflow-y-auto">
            ...
            <div className="flex gap-2 mb-6 overflow-x-auto pb-2 scrollbar-hide">
              {['All', 'Popular', 'Running Low', 'Low Stock', 'On Sale', 'Sold Out'].map(tag => (
                <button 
                  key={tag} 
            ...
            {filteredItems.length > 0 ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                {filteredItems.map((item) => (
                  <div key={item.id} className="bg-white p-4 rounded-lg shadow-sm hover:shadow-md transition cursor-pointer group relative overflow-hidden border border-gray-200">
                    {item.status && (
                      <span className={`absolute top-3 left-3 text-xs ${getStatusColor(item.status)} px-2 py-1 rounded-full z-10`}>
                        {item.status}
                      </span>
                    )}
                    ...
                    <div className="flex justify-between items-center mt-2">
                      <div>
                        ...
                        {item.discount && (
                          <span className="text-xs text-gray-500 line-through ml-1">${item.discount.toFixed(2)}</span>
                        )}
                      </div>
                      <button 
              ...
              <div className="flex flex-col items-center justify-center h-64 text-gray-400 bg-white rounded-lg shadow-sm border border-gray-200">
                ...
                <p className="text-gray-500">No items found for "{searchQuery}" in {selectedCategory}</p>
              </div>
            )}
          </section>
 ...
 className="flex-1 flex items-center justify-center gap-2 text-sm bg-white border border-gray-300 hover:border-black px-3 py-2 rounded-lg transition">
              ...
              <div className="space-y-2">
                <button  onClick={() => setShowModal(true)} className="w-full flex items-center justify-center gap-2 text-sm bg-white border border-gray-300 hover:border-black px-4 py-3 rounded-lg transition">
...
{isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="bg-white rounded-xl w-full max-w-lg shadow-lg p-6 relative">
            ...
            {/* Title */}
            <h2 
              className="text-lg font-semibold">Add Customer</h2>
      ...
      <div className="p-6 space-y-6 ">
        ...
        <div>
          ...
          <div className="flex gap-4">
            <button className="border p-3 rounded-lg border-black bg-black text-white font-medium">Uber Eats</button>
            <button className="border p-3 rounded-lg hover:border-black">Uber Eats</button>
            <button className="border p-3 rounded-lg hover:border-black">Lyft</button>
          </div>
        </div>
        ...
        <div>
          ...
          <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
            {['Today', 'Sat 26 Jul', 'Sun 27 Jul', 'Mon 28 Jul', 'Tue 29 Jul'].map((date) => (
              <button
                key={date}
        ...
        <div>
          <label className="block text-sm font-medium mb-2">Delivery Instruction</label>
          <textarea
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg w-full max-w-3xl overflow-hidden shadow-lg animate-fade-in">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 px-6 pb-4">
              {[
                { type: "VIP Member", color: "text-pink-600", tagColor: "bg-pink-100" },
                { type: "All Member", color: "text-blue-600", tagColor: "bg-blue-100" },
                { type: "VIP Member", color: "text-pink-600", tagColor: "bg-pink-100" },
              ].map((item, idx) => (
                <div
                  key={idx}
                ...
                >
                  ...
                  <p className="text-xs text-gray-500">Valid until 12 June 2023</p>
...
{isOpen && isCreateModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center  bg-black/50 backdrop-blur-sm">
          <div className="bg-white rounded-xl w-full max-w-md shadow-lg p-6 relative">
            ...
            {/* Title */}
            <h2 onClick={() => setIsCreateModalOpen(true)}
className="text-lg font-semibold mb-4">Create New Customer</h2>
